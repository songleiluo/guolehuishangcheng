...
//移动的物体(DOM元素)
//样式属性名；
//方向(1表示正向，由小到大；-1表示反向，由大到小)
//起始位置
//结束位置
//步长
//频率(时间间隔)

function movewu_01(domObj,attr,direction,startP,endP,inc,timeSpace){
	let currPos = startP;
...
//步长
//频率
function movewu_02(domObj,attr,endP,inc,timeSpace){
	let startP=parseInt(getStyle(domObj,attr));
	let direction =endP>startP?1:-1;
	movewu_01(domObj,attr,direction,startP,endP,inc,timeSpace)
}
...
function fadeInOut(domObj,endValue,inc,timeSpace){
	...
	let myTimer = setInterval(function(){
		//1.改变数据
		currVal =currVal+inc*direction;
		//2.边界处理
		if(direction>0?currVal>=endValue:currVal<=endValue){
			window.clearInterval(myTimer);
			currVal=endValue;
		}
		//3.外观呈现
		domObj.style.opacity=currVal;
	},timeSpace);
}
//淡入
function fadeIn(donObj,inc,timeSpace){
	fadeInOut(domObj,1,inc,timeSpace);
}
//淡出
function fadeOut(donObj,inc,timeSpace){
